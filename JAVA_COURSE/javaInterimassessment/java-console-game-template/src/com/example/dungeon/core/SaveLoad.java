...
public class SaveLoad {
    ...
    try (BufferedReader r = Files.newBufferedReader(SAVE)) {
        ...
        // безопасная обработка даже если данных не хватает
        if (pp.length >= 2) p.setName(pp[1]);
        if (pp.length >= 3) {
            try { p.setHp(Integer.parseInt(pp[2])); } 
            catch (NumberFormatException e) { p.setHp(10); }
        }
        if (pp.length >= 4) {
            try { p.setAttack(Integer.parseInt(pp[3])); } 
            catch (NumberFormatException e) { p.setAttack(3); }
        }
